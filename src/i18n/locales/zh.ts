export const zh = {
  ...
  config: {
    ...
    sections: {
      ...
      input: "摇杆输入", inputDescription: "使用左右独立的缩放径向死区修正中心漂移。",
    },
    microphoneEnabled: "启用麦克风", speakerEnabled: "启用扬声器", speakerRoute: "扬声器路由",
    hapticsGainQ8: "触觉增益（Q8）", audioBufferLength: "手柄音频缓冲长度", cpuGovernor: "CPU governor", cpuProfile: "CPU 档位",
    manualCpuMhz: "自定义 CPU 频率（MHz）", autoReconnectEnabled: "自动重连手柄", statusLedEnabled: "启用 M61 状态灯", statusLedBrightnessPercent: "状态灯亮度（%）",
    ...
    stickPreview: {
      left: "左摇杆", right: "右摇杆", waiting: "等待摇杆输入",
      output: "显示当前固件死区修正后的实时 USB 输出；绘制频率限制为屏幕刷新率。",
    },
    help: {
      ...
      audioBufferLength: "范围 16–127。该字节作为缓冲提示写入发给 DualSense 的音频报告，不会额外分配 M61 内存。较小值可能降低手柄侧延迟，但更容易暴露音频或触觉异常；经过验证的发布默认值为 48。",
      ...
      statusLedBrightnessPercent: "直接调整 BL616 硬件 PWM，不增加周期性 CPU 任务；发布默认亮度为 12%。",
    ...
    warnings: {
      ...
      audioBufferTuning: "48 是经过验证的发布值。其他手柄缓冲提示会在延迟和稳定性之间取舍，可能暴露音频或 HD 触觉异常，请测试后再保存。",
  ...
  actions: {
    ...
    savePerformanceConfirm: "当前配置启用了麦克风、实时升频、384/400 MHz 档位或非默认音频缓冲提示。它可能增加负载、功耗、温升、音频异常或卡顿风险，确认保存到 Flash 吗？",
  ...
  diagnostics: {
    bluetooth: "手柄蓝牙", usb: "USB 已配置", usbSuspend: "USB 已挂起", controllerSaved: "已保存手柄",
    configLoaded: "Flash 配置已加载", pairing: "配对模式", discovery: "正在扫描", cpu: "当前/请求 CPU",
    management: "管理序号/错误", usbDrops: "USB 输入丢弃", hostDrops: "主机报告丢弃",
    audioDrops: "音频队列丢弃", codecErrors: "Codec 错误", featureQueues: "Feature 读/写队列",
    audioQueues: "触觉/扬声器队列", yes: "是", no: "否", unavailable: "不可用",
  },
  validation: {
    schemaVersion: "M61 配置 schema 无效", capabilities: "M61 能力位无效", speakerRoute: "扬声器路由无效", cpuGovernor: "CPU governor 无效",
    ...
    statusLedBrightnessPercent: "状态灯亮度必须在 1%–100%", audioBufferLength: "音频缓冲长度必须在 16–127",
  },
  errors: {
